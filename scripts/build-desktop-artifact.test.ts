...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("keeps the fork product name distinct across update channels", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code Auto");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code Auto");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Code Auto 1.2.3 Installer",
      ...
      // entry advertises the fork-specific scheme for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Code Auto", schemes: ["t3code-auto", "t3code-auto-dev"] },
      ]);
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.codepanda.t3code-auto",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.codepanda.t3code-auto</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.codepanda.t3code-auto");
